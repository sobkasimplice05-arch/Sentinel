...
def test_render_discord_contains_key_metrics():
    metrics = {
        "git": {"commit_count": 4, "autonomous_commit_count": 2, "memory_only_commit_count": 1, "source_promotion_count": 1},
        ...
        "active_cooldowns": ["GROQ"],
        "self_modification": {"decision": "PROVIDER_COOLDOWN", "provider": "GROQ", "changed_files": ["autonomy_kernel.py"], "candidate_score": 0.85},
    }
    ...
    assert "mémoire seule : **1**" in text
    assert "code promu : **1**" in text
    assert "Cooldowns actifs : `GROQ`" in text
