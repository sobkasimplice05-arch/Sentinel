...
Pour activer une génération effective de patches, Sentinel utilise `SELF_MODIFICATION_MODEL_URL`, `MODEL_API_URL` ou `OLLAMA_BASE_URL` lorsqu’un endpoint est défini. En mode `SELF_MODIFICATION_PROVIDER=auto`, elle essaie d’abord l’endpoint configuré, puis NVIDIA, Google Gemini, Groq, Replicate si un modèle/version est configuré, un endpoint générique et enfin Hugging Face, en ignorant les fournisseurs en cooldown. Les endpoints par défaut sont NVIDIA NIM (`https://integrate.api.nvidia.com/v1/chat/completions`), Gemini (`https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent`), Groq (`https://api.groq.com/openai/v1/chat/completions`) et Hugging Face Inference. Une installation Ollama peut être sélectionnée avec `SELF_MODIFICATION_MODEL_URL` ou `OLLAMA_BASE_URL`.
